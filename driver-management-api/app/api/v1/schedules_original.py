...
from app.crud import (
    get_schedules as crud_get_schedules, get_schedule_by_id as crud_get_schedule_by_id,
    create_schedule as crud_create_schedule, update_schedule as crud_update_schedule,
    delete_schedule as crud_delete_schedule,
    get_driver_by_id, get_vehicle_by_id,
...
async def get_schedules(
    ...
    """获取排班列表"""
    try:
        schedules = await crud_get_schedules(
            db=db,
            skip=skip,
            limit=limit,
            driver_id=driver_id,
...
async def create_schedule(
    ...
    try:
        # 验证司机是否存在
        driver = await get_driver_by_id(db, schedule.driver_id)
        if not driver:
            raise HTTPException(status_code=404, detail="司机不存在")
            
        # 验证车辆是否存在（如果指定了车辆）
        if schedule.vehicle_id:
            vehicle = await get_vehicle_by_id(db, schedule.vehicle_id)
            if not vehicle:
                raise HTTPException(status_code=404, detail="车辆不存在")
                
        # 检查时间冲突
        existing_schedules = await crud_get_schedules(
            db=db,
        ...
        for existing_schedule in existing_schedules:
            if existing_schedule.status in ["scheduled", "in_progress"]:
                if (existing_schedule.start_time < schedule.end_time and 
                    existing_schedule.end_time > schedule.start_time):
                    raise HTTPException(status_code=400, detail="该时间段已有排班冲突")
        
        db_schedule = await crud_create_schedule(db=db, **schedule.dict())
